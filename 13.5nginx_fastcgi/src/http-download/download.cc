...
#include "debug.h"
#include "md5.h"
// 多线程共用一个回调写入函数 真正返回的字节数量 size*nmemb
           ...
           : m_url(url), m_fileName(fileName)
{
    ...
    m_checkopt = 0;
    m_size = GetSize();  // 获取文件大小
    m_workJobNum = 0;
    m_jobNum= jobNum > 0 ? jobNum : 1;  // 下载线程数量
    m_jobs = new download_jobs_t[m_jobNum];         
    fragmentSize = (m_size + m_jobNum - 1) / m_jobNum;
    printf("m_size:%ld, m_jobNum:%d, fragmentSize:%ld.\n", 
        m_size, m_jobNum, fragmentSize);        // fragmentSize每个分块的最大下载数据
    for (index = 0; index < m_jobNum; ++index)
    {
        m_jobs[index].download = this;
        m_jobs[index].fd = m_fd;        // 写入同一个文件
        m_jobs[index].start = index * fragmentSize;     // 写入文件的起始位置
        m_jobs[index].end = (index + 1) * fragmentSize - 1;
        m_jobs[index].end = m_jobs[index].end >= m_size ? m_size-1 : m_jobs[index].end; // 写入文件的结束位置
        ...
        curl_easy_setopt (curl, CURLOPT_NOSIGNAL, 1L);      // libcurl如果检查到设置的timeout < 1s 那么会直接发出一个信号说 “已经超时了”如果要使用毫秒超时 那么还需要关闭这个signal功能，即设置中需要多一个
        curl_easy_setopt (curl, CURLOPT_LOW_SPEED_LIMIT, 1L);   // 当 transfer speed 小于 1字节/秒 时 将会在 5秒内 abort 
        curl_easy_setopt (curl, CURLOPT_LOW_SPEED_TIME, 5L);  
        curl_easy_setopt (curl, CURLOPT_RANGE, range);          // 下载内容的范围
        printf("range:%s.\n", range);
    }
}
...
long CDownload::GetSize()
{
    ...
    res = curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD, &size); // 获取文件大小
    if (CURLE_OK != res)
    {
